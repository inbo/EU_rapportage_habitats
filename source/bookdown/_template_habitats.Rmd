...
habitattypes %>%
  ...
  filter(!(main_type %in% excluded_habitats | str_detect(main_type, "rbb")),
        id == this_hab) %>%
...
data_opp %>% filter(HT %in% 
           ...
           !(HT %in% excluded_habitats | str_detect(HT, "rbb"))) %>%
...
data_area %>% filter(hab == this_hab) %>%
  ...
  left_join(trend_translate, by = c("trend" = "trendeng")) %>%
  left_join(status_translate, by = c("conclusie" = "statuseng")) %>%
  mutate(area = as.numeric(str_replace(area,',','.')) * 100,#km² -> ha
...
colors_status <- c("Ongunstig" = INBOtheme::inbo_rood, 
                   "Gunstig" = INBOtheme::inbo_groen, 
                   "Onbekend" = INBOtheme::inbo_grijs)
data_stathab <- data_statushabitat %>%
  filter(Habitattype %in% 
           unlist(habitattypes[habitattypes$id == this_hab, "main_type"]) & 
           !(Habitattype %in% excluded_habitats | str_detect(Habitattype, "rbb")) & 
...
patchwork::wrap_plots(plots, ncol = 1, guides = "collect", heights = heights) & 
  theme(legend.position = "bottom")
```
...
for (column in 3:(3 + n_distinct(data_ss$Habitattype) - 1)) {
  for (row in 1:3) {
    t[row, column + 1] <- cell_spec(t[row, column + 1], "latex", 
                                    background = temp_colors[row, column])
  }
}
...
### Trends en toestand in regionale toestand binnen SBZ-H vs Vlaanderen

Mogelijks nog aanvullen

### Trends in regionale toestand
...
main_types <- unlist(habitattypes[habitattypes$id == this_hab, "main_type"])

#Work in progress (moesten we ooit verschil binnen en buiten SBZH willen bekijken)
data_lsvi_x <- data_aandeelgunstig %>%
  filter(Habitattype %in% main_types,
           !(Habitattype %in% excluded_habitats  | str_detect(Habitattype, "rbb")),
           (TypeResultaat == "Habitattype" | is.na(TypeResultaat)) | 
           (TypeResultaat == "SBZH" & SBZH == "Binnen"),
         Versie == "Versie 3") |> 
  dplyr::select(!c(Criterium, Belang)) %>%
  #xe9 en xeb worden niet ondercheiden
  mutate(Indicator = str_replace(Indicator, "\xeb", "XXXXX"),
         Indicator = str_replace(Indicator, "\xe9", "YYYYY"),
         Indicator = str_replace(Indicator, "XXXXXYYYYY", "éé"),
         Indicator = str_replace(Indicator, "XXXXX", "ë")) %>%
...
data_lsvi <- data_lsvi_x |> filter(TypeResultaat != "SBZH")
if (nrow(data_lsvi) > 0) {
  ...
  #mogelijks nog overlappende lijnen door "Periode"
  plots <- lapply(X = habs, 
...
Mogelijks nieuwe input



### Trends en toestand in LSVI binnen SBZ-H vs Vlaanderen
