...
library(tidyverse)
library(rprojroot)
library(parsermd) #convert_docx_to_rmd, parse_rmd
...
for (hoofdstuk in hoofdstukken) {
  ...
  assertthat::assert_that(
    sum(!(paragrafen %in% rmd$sec_h1)) == 0 ,
    msg =  sprintf("These chapters are missing in the word file:%s",
                   paste(paragrafen[!(paragrafen %in% rmd$sec_h1)],
                         collapse = ", ")))
  ...
  for (x in paragrafen) {
    ...
    as_document() %>%
    str_replace("\\\\", "\\") %>% #avoid double backslash is print in the md file
